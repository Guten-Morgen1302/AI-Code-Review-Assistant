...
sequenceDiagram
    ...
    WH->>Server: Forward validated event
    
    Server->>GHS: Fetch PR diff & metadata
    GHS->>GH: GET /repos/{owner}/{repo}/pulls/{pr}
    GH-->>GHS: Return PR data + diff
    
    GHS->>Parser: Parse diff into structured format
    Parser->>Parser: Filter auto-generated files
    Parser->>Parser: Extract changed lines per file
    
    Server->>Cache: Check custom rules (cache key: repo_id)
    alt Cache Hit
        Cache-->>Server: Return cached rules (50ms)
    else Cache Miss
        Server->>DB: Fetch rules from MongoDB
        DB-->>Server: Return rules
        Server->>Cache: Populate cache (async)
    end
    
    Server->>Rules: Load custom rules
    Rules->>Rules: Validate rule syntax
    
    Server->>AI: Initiate review process
    AI->>AI: Build GPT-4 prompt with:<br/>- Code diff<br/>- File metadata<br/>- Custom rules<br/>- Review focus areas
    
    AI->>GPT: POST /v1/chat/completions<br/>(streaming, 8k tokens)
    Note over AI,GPT: ~150ms processing time
    
    alt Success
        GPT-->>AI: Return structured JSON review<br/>(issues, severity, line numbers)
        AI->>AI: Parse & validate response
        AI->>Cache: Cache results (TTL: 1h)
        AI->>GHS: Format GitHub comments
        GHS->>GH: POST review comments to PR
        GH-->>Dev: Display review in PR
    else Timeout (>30s)
        GPT--xAI: Timeout
        AI->>GPT: Retry with GPT-3.5-turbo (faster)
        alt GPT-3.5 Success
            GPT-->>AI: Return review
        else GPT-3.5 Fails
            AI->>Rules: Fallback to rule engine only
            Rules-->>AI: Return basic review
        end
    else Rate Limit (429)
        GPT--xAI: Rate limit exceeded
        AI->>Queue: Queue job for retry (60s)
    end
    
    Server->>DB: Store review metadata<br/>(timestamp, files, issues, tokens)
    Server->>WS: Broadcast "Review Complete"
    WS-->>Dev: Real-time dashboard update
    
    Server->>Queue: Add notification job
    Queue->>Notify: Process notification
    Notify->>Dev: Send Slack/Email alert
    
    Note over Server,GPT: Total pipeline: 180-250ms
    Note over Cache: Redis caching achieves<br/>200ms avg response time
